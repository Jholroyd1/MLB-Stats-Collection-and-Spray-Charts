...
# --- Field geometry (copied from your blank field script) ---
...
# The columns are not named in the CSV, so we must specify them
columns = [
...
ax.scatter(data['coord_x'], data['coord_y'], color='red', edgecolors='black', s=60, zorder=20, alpha=0.7)
...
ax.axis('off')
plt.tight_layout()
plt.savefig('../data/ohtani_2025_spray_chart_on_field.png', dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.show()
