...
# Center between original and flipped x
plot_x = 0.5 * (data['coord_x'] - data['coord_x']) + 0.5 * (data['coord_x'] * -1)
plot_x = 0.5 * (data['coord_x'] + (-data['coord_x']))
plot_x = 0.0  # This would collapse all points to x=0, which is not correct
# Instead, average the original and flipped x
plot_x = 0.5 * (data['coord_x'] + (-data['coord_x']))
# But this is always zero. Instead, try a weighted average between original and flipped
plot_x = 0.5 * data['coord_x'] + 0.5 * (-data['coord_x'])
# This is also always zero. Instead, shift the flipped x by a constant offset
# Let's try shifting the flipped x by the mean difference between the two
flipped_x = -data['coord_x']
mean_shift = (data['coord_x'].mean() + flipped_x.mean()) / 2
plot_x = flipped_x + mean_shift
plot_y = data['coord_y']
...
ax.axis('off')
plt.tight_layout()
plt.savefig('../data/ohtani_2025_spray_chart_on_field_centered.png', dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.show()
