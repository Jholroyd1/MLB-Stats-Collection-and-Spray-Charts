...
# Print diagnostics
print('Rotated coord_x: min', plot_x.min(), 'max', plot_x.max())
print('Rotated coord_y: min', plot_y.min(), 'max', plot_y.max())
print('Fence y: min', fence_points[:,1].min(), 'max', fence_points[:,1].max())

# Highlight home runs
is_hr = data['event_type'].str.lower().str.contains('home_run')
ax.scatter(plot_x, plot_y, color='red', edgecolors='black', s=60, zorder=20, alpha=0.7, label='All batted balls')
ax.scatter(plot_x[is_hr], plot_y[is_hr], color='gold', edgecolors='black', s=80, zorder=21, label='Home runs')
...
plt.savefig('../data/ohtani_2025_spraychart_diagnostic.png', dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.show()
